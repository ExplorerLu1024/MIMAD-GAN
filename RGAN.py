...
with open("loss.txt", mode="a", encoding='utf-8') as f:
    ...
    # for epoch in range(1):
        ...
        line = str(epoch) + " " + str(D_loss_curr) + " " + str(G_loss_curr)
        f.write('%d\t%.4f\t%.4f' % (epoch, D_loss_curr, G_loss_curr))
        f.write("\n")
        ...
        #
        # # MMD # reset ADAM variables sess.run(tf.initialize_variables(sigma_opt_vars)) sigma_iter = 0 that_change =
    # sigma_opt_thresh * 2 old_that = 0 while that_change > sigma_opt_thresh and sigma_iter < sigma_opt_iter: new_sigma,
    # that_np, _ = sess.run([sigma, that, sigma_solver], feed_dict={eval_real_PH: eval_eval_real, eval_sample_PH:
    # eval_eval_sample}) that_change = np.abs(that_np - old_that) old_that = that_np sigma_iter += 1 opt_sigma =
    # sess.run(sigma) try: mmd2, that_np = sess.run(mix_rbf_mmd2_and_ratio(eval_test_real, eval_test_sample,
    # biased=False, sigmas=sigma)) except ValueError: mmd2 = 'NA' that = 'NA'
        #
        #     MMD[epoch, ] = mmd2
