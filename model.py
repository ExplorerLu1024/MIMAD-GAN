...
def sample_T(batch_size, batch_idx):
    samples_aaa = np.load('./data/samples_aa.npy',allow_pickle=True)
    num_samples_t = samples_aaa.shape[0]
    labels_aaa = np.load('./data/labels_aa.npy',allow_pickle=True)
...
def sample_TT(batch_size):
    samples_aaa = np.load('./data/samples_aa.npy',allow_pickle=True)
    labels_aaa = np.load('./data/labels_aa.npy',allow_pickle=True)
    idx_aaa = np.load('./data/idx_aa.npy',allow_pickle=True)
    T_indices = np.random.choice(len(samples_aaa), size=batch_size, replace=True)
...
def train_epoch(epoch, samples, labels, sess, Z, X, D_loss, G_loss, D_solver, G_solver,
    ...
    loss_dict = {'D_loss': D_loss_curr, 'G_loss': G_loss_curr}
    avg_params = loss_dict
...
def GAN_loss(Z, X, generator_settings, discriminator_settings):
    ...
    D_loss_real = tf.reduce_mean(tf.math.exp(1 - D_real))
    D_loss_fake = tf.reduce_mean(tf.math.exp(D_fake))
    D_loss = D_loss_real + D_loss_fake

    # Calculate loss for generator
    G_loss = tf.reduce_mean(tf.math.exp(D_fake))
...
def GAN_solvers(D_loss, G_loss, learning_rate, batch_size, total_examples, l2norm_bound, batches_per_lot, sigma, dp=False):
    ...
    if dp:
        print('Using AdamW to train discriminator!')
        ...
        D_solver = dp_optimizer.DPAdamGaussianOptimizer(learning_rate,
                                                        [eps, delta],
        ...
        D_solver = tf.contrib.opt.AdamWOptimizer(weight_decay=weight_decay,learning_rate=learning_rate).minimize(D_loss_mean_over_batch, var_list=discriminator_vars)
